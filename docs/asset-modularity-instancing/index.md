...
* Explain the concept of instancing in OpenUSD and its relationship to the {term}`composition <Composition>` system
...
* Utilize {term}`variant sets <Variant Set>` to implement geometric and material variations in {term}`instanceable <Instanceable>` assets
